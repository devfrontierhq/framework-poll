...
- [ ] 1.1 使用 Vite 建立 React TypeScript 專案
- [ ] 1.2 安裝 Tailwind CSS 並設定配置檔
- [ ] 1.3 安裝 shadcn/ui 元件庫
- [ ] 1.4 安裝 Zustand 狀態管理函式庫
- [ ] 1.5 安裝 date-fns 日期處理函式庫
- [ ] 1.6 建立 .env 檔案並設定 VITE_ADMIN_SECRET 環境變數
- [ ] 1.7 安裝並設定 ESLint、Prettier 與 Husky（TypeScript + React 規則 + pre-commit hooks）
- [ ] 1.8 安裝 Vitest 與 React Testing Library
- [ ] 1.9 安裝 React Compiler 與 vite-plugin-babel 實作自動效能最佳化

## 2. 型別定義與工具函式

- [ ] 2.1 建立 types/dotBoard.ts 定義 Category data model 與 Dot data model、CsvRow 型別
...
- [ ] 2.6 在 utils/csv.ts 實作 downloadCsv 函式下載 UTF-8 with BOM 編碼的 CSV file format，確保 no data loss
...
- [ ] 3.4 建立 dots store 的 [categoryId, deletedAt] 複合索引用於 query optimization
