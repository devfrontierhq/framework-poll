...
- 建立全新的互動式計分板應用程式
- 支援任何使用者新增圓點進行投票
- 提供管理員模式進行版塊管理（批次新增、編輯、刪除）
- 提供一鍵初始化功能，讓開發者可快速建立三大框架板塊（React、Vue、Angular）以便測試投點功能
...
- `category-management`: 調整平板與桌機版面——固定 3 欄格子，≤3 筆時格子長方形撐滿高度，≥4 筆時格子改為正方形（依容器寬自動計算）並開啟垂直捲軸；新增版塊對話框改為多列批次建立模式，管理員可一次輸入多個標題與顏色後快速建立
