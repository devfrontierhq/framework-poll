...
- [x] 3.2 為 `computeGridLayout` 新增單元測試：0 顆點回傳空陣列、1 顆點落在左上角（single dot arrangement）、N 顆點所有 ratio 在 [0, 1]（dots fill available space）、間距不足時自動增加 cols（dynamic spacing when dots are too dense）、排序正確
...
- [x] 5.3 手動驗證：點擊「整齊排列」後點點整齊排列、reload 後位置保留（positions visible after reload）、各板塊點點數不同時各自正確排列、非管理員看不到按鈕（auto-arrange requires admin mode）
