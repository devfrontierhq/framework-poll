...
- [x] 6.3 建立 CategoryGrid.tsx 實作 display categories 響應式 Grid 佈局（桌機和平板固定 3 欄，≤3 筆長方形撐滿高度，>3 筆正方形加垂直捲軸、手機 1 欄）
...
- [x] 6.8 建立 useLongPress hook（src/hooks/useLongPress.ts）：觸控裝置長按（500ms）事件處理，支援提前放開取消與手指移動超過 moveThreshold 取消
- [x] 6.9 在 CategoryCard 整合 useLongPress：觸控裝置（平板）長按 dot 時顯示 tooltip，桌機維持 hover 行為
- [x] 6.10 測試 useLongPress hook：驗證 500ms 觸發、提前放開取消、手指移動超過 moveThreshold 取消
...
- [x] 7.5 在 EmptyState.tsx 以 `import.meta.env.DEV` 條件渲染「建立預設框架板塊」按鈕，production 環境不顯示
- [x] 7.6 更新 EmptyState 的空狀態文字，production 環境顯示適合一般使用者的說明（例如「等待管理員建立版塊」）
- [x] 7.7 更新相關測試：驗證 dev 模式顯示按鈕、production 模式隱藏按鈕
...
- [x] 9.10 修改 AddCategoryDialog.tsx 改為多列模式：每列包含標題輸入與顏色選擇器
- [x] 9.11 實作「新增一列」按鈕，點擊後在最後追加一列空白輸入列
- [x] 9.12 實作「刪除列」按鈕，僅在列數 ≥ 2 時顯示，刪除對應列
- [x] 9.13 修改送出邏輯：過濾標題為空的列，對每列呼叫 addCategory action
- [x] 9.14 更新 AddCategoryDialog 相關測試：單列行為維持不變、多列批次建立、空標題列過濾
...
- [x] 11.1 測試 store actions 的單元測試
- [x] 11.2 測試管理員模式開關的 UI 互動
- [x] 11.3 測試權限控制：非管理員無法看到管理功能
- [x] 11.4 測試響應式佈局在不同螢幕尺寸的表現
- [x] 11.5 測試空狀態顯示與引導文字
- [x] 11.6 測試錯誤提示（例如密鑰錯誤）
- [x] 11.7 驗證所有 spec 需求都已實作完成
