...
export function CategoryCard({ category, categoryDots }: CategoryCardProps) {
  ...
  const handleDotClick = (e: React.MouseEvent<HTMLButtonElement>, dot: Dot) => {
    if (isAdminUnlocked) {
      e.stopPropagation()
      setSelectedDot(dot)
  ...
  const handleAreaClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    const { xRatio, yRatio } = calculateRelativeCoordinates(clickX, clickY, rect.width, rect.height)
  ...
  const handleSubmitDot = async (name: string) => {
    if (pendingCoordinates) {
      await addDot(category.id, name, pendingCoordinates.xRatio, pendingCoordinates.yRatio)
  ...
  return (
    <div
      className="flex min-h-[300px] flex-col rounded-lg border border-slate-200 bg-white shadow-sm md:h-full md:min-h-0"
    >
